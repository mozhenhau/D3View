...
//  Neighborhood
//
//  Created by mozhenhau on 14-7-8.
//  Copyright (c) 2014 Ray. All rights reserved.
//  滚动图
//  使用方法：imgArr加入图片后，initScrollView，   修改样式用initStyle
...
class D3ScrollView: UIControl ,UIScrollViewDelegate{
    ...
    required init(coder aDecoder: NSCoder) {
        ...
        //默认样式
        d3ImgView = UIImageView(image: UIImage(named:D3Cons.defaultImg))
        ...
        d3MaskView = UIView()
        d3MaskView.backgroundColor = D3Cons.grayColor
    ...
    func initScrollView()
    {
        ...
        if imgArr.count <= 1
        {
            ...
            d3ScrollView!.contentSize = CGSizeMake(d3ScrollView!.frame.size.width * 3,d3ScrollView!.frame.size.height)
            ...
            d3PageControl!.pageIndicatorTintColor = UIColor.whiteColor()
            d3PageControl!.currentPageIndicatorTintColor = D3Cons.defaultColor
