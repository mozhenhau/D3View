...
class RefreshHeaderView: RefreshBaseView {
    ...
    var headerPullDescription:String = "下拉可以刷新"
    var headerReleaseDescription:String = "松开立即刷新"
    let userDefault = NSUserDefaults.standardUserDefaults()

    class func footer()->RefreshHeaderView{
        let x:CGFloat = 0
        let y:CGFloat = 0
        var footer:RefreshHeaderView  = RefreshHeaderView(frame: CGRectMake(x, y, UIScreen.mainScreen().bounds.width, RefreshViewHeight))
        return footer
    }
    ...
    didSet{
        userDefault.setObject(lastUpdateTime, forKey: RefreshHeaderTimeKey as! String)
    ...
    override init(frame: CGRect) {
        ...
        if  (userDefault.objectForKey(RefreshHeaderTimeKey as! String) == nil)  {
            self.lastUpdateTime = NSDate()
        }
        else {
            self.lastUpdateTime = userDefault.objectForKey(RefreshHeaderTimeKey as String) as! NSDate
        }
        self.updateTimeLabel()
    }
    ...
    didSet{
        switch State{
        case .Normal:
            self.statusLabel?.text = headerPullDescription
            ...
            break
        case .Pulling:
            self.statusLabel?.text = headerReleaseDescription
            UIView.animateWithDuration(RefreshSlowAnimationDuration, animations: {
