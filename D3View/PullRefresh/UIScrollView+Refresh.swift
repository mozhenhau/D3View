...
extension UIScrollView {
   ...
   func addFooterWithCallback( callback:(() -> Void)!){
    ...
    /** 修改footer的加载描述*/
    func addFooterWithCallbackDescription(pull:String,release:String,callback:(() -> Void)!){
        var footer:RefreshFooterView = RefreshFooterView.footer()
        footer.footerPullDescription = pull
        footer.footerReleaseDescription = release
        self.addSubview(footer)
        footer.beginRefreshingCallback = callback
    ...
    /** 修改header的加载描述*/
    func addHeaderWithCallbackDescription(pull:String,release:String,callback:(() -> Void)!){
        var header:RefreshHeaderView = RefreshHeaderView.footer()
        header.headerPullDescription = pull
        header.headerReleaseDescription = release
        self.addSubview(header)
        header.beginRefreshingCallback = callback
        header.addState(RefreshState.Normal)
    }
